...
list_tasks = {}
list_tasks["ls_mkdir_touch"] = Task(
    name="ls_mkdir_touch", max_points=30, number_tests=4)
...
def calculate_score() -> int:
    ...
    return calculate_score_without_memory() +\
        (memory_score if len(memory_shame_list) ==
         0 and calculate_score_without_memory() >= 50 else 0)
...
if not os.path.exists(INPUT_FILE):
    sys.stderr.write(
        "Could not find the inputs file.....how did you manage that?")
    sys.exit(-1)


print("\n======================= Tema 1 IOCLA =======================\n")

regular_tests = [x for x in sorted(
    os.listdir(INPUT_FILE)) if not x.startswith("mv_")]
...
for name_file in regular_tests:
    ...
    with open(DEV_FILE + "dev_" + name_file, "r") as dev:
        if dev.read().strip() == result:
            print("TEST " + name_file + "." *
                  (size_header - len(name_file) - 12) + " PASSED")
            add_passed_test(name_file)
            numPassed += 1
        else:
            print("TEST " + name_file + "." *
                  (size_header - len(name_file) - 12) + " FAILED")
    ...
    with open("memCheck", "r") as mem:
        ...
        if "All heap blocks were freed" not in mem_str:
            ...
            memory_shame_list.append(test_tuple(
                name_file, "not enough memory allocated string"))

print("\n========================== BONUS ===========================\n")

bonus_tests = [x for x in sorted(
    os.listdir(INPUT_FILE)) if x.startswith("mv_")]
...
if len(memory_shame_list) == 0 and calculate_score_without_memory() >= 50:
    if numPassed == len(os.listdir(INPUT_FILE)):
        print(
            f"Very few people have gone as far as you have.{nl}Good Job! I hope you are proud of yourself the least I can give {nl}you is {memory_score}p!")
    else:
        print(f"No memory problems, really good job! - {memory_score}p!")
else:
    print(
        f"At least 50 points are necessary in order to get the points for memory.{nl}")
